...
for snr = SNR
    for r = 1:size(Rows, 1)
        ...
        tempp = size(Y_data);
        x_reshaped = reshape(permute(x_real_imag, [2, 1, 3]), [tt(1:3), 1]);
        
        Y_ = H_data.*x_reshaped;
        
        Noise = Y_data -Y_; % 14 x 612 x 2 x Nsamples
        ...
        if dataset_params.staticChan == 1
            ...
            save(save_folder,'H_data', 'H_linear_data', 'H_equalized_data', 'Y_data', 'H_practical_data', 'Noise', "dataset_params",'-v7.3');
        end
    end
end
